...
def _intersect_range(lhs, rhs):
    nextlhs = iter(lhs).__next__
    nextrhs = iter(rhs).__next__
    try:
        lmin, lmax = nextlhs()
        rmin, rmax = nextrhs()
...
class Range(Condition):
    ...
    def gen(cls, expr, *ranges):
        # normalize ranges as list of (mini,maxi) tuples
        ranges = list(ranges)
        offset = 0
        for i in range(len(ranges)):
            try:
                ...
                mini = maxi = ranges[i]
            if mini is not None: mini = int(mini)
            if maxi is not None: maxi = int(maxi)
            if mini is not None and maxi is not None and mini > maxi:
                offset += 1
            else:
                ranges[i - offset] = mini, maxi
        else:
        ...
        if expr.simple():  # reduces to constant
            ...
            for mini, maxi in ranges:
                if (mini is None or mini <= v) and (maxi is None or v <= maxi):
                    return Always()
            else:
                return Never()
        ...
        while True:
            if isinstance(expr, LinearExpr):
                if expr[0] != 0:  # mini <= const + ... <= maxi
                    const = expr[0]
                    expr = LinearExpr(*expr[1:])
                    for i in range(len(ranges)):
                        mini, maxi = ranges[i]
                        if mini is not None: mini -= const
                        if maxi is not None: maxi -= const
                        ranges[i] = mini, maxi
                    continue

                if len(expr) == 2:  # mini <= 0 + coeff * term <= maxi
                    coeff = abs(expr[1][0])
                    ...
                    for i in range(len(ranges)):
                        ...
                        if changesign:
                            mini, maxi = maxi, mini
                            if mini is not None: mini = (-mini - 1) // coeff + 1
                            if maxi is not None: maxi = -maxi // coeff
                        else:
                            if mini is not None: mini = (mini - 1) // coeff + 1
                            if maxi is not None: maxi = maxi // coeff
                        if mini is not None and maxi is not None and mini > maxi:
            ...
            if isinstance(expr, ExactDivisionExpr):  # mini <= term / div <= maxi
                if expr.rhs.simple():
                    rhs = int(expr.rhs)
                    expr = expr.lhs
                    for i in range(len(ranges)):
                        mini, maxi = ranges[i]
                        if mini is not None: mini *= rhs
                        if maxi is not None: maxi *= rhs
                        ranges[i] = mini, maxi
                    continue
        ...
        ranges.sort(key=lambda min_max: (min_max[0] is not None, min_max[0], min_max[1] is None, min_max[1]))
        ...
        if len(result) == 0:
            ...
            return Condition.gen(cls, expr, *result)
    ...
    def compactrepr(self, prec=0):
        ...
        if len(self.ranges) == 1:
            ...
            for min, max in self.ranges:
                if min is None:
                    clauses.append('..%r' % max)
                elif max is None:
                    clauses.append('%r..' % min)
                elif min == max:
                    clauses.append(repr(min))
                else:
                    clauses.append('%r..%r' % (min, max))
...
class Conjunction(Condition, tuple):
        ...
        for clause in clauses:
            ...
            for iclause in clause:
                if isinstance(iclause, Always):
                    ...
                    try:
                        rr = ranges[iclause.expr]
                    except KeyError:
                        rr = [(None, None)]
                    ranges[iclause.expr] = _intersect_range(rr, newrr)

        for expr, rr in list(ranges.items()):
    ...
    def __invert__(self):
        return Disjunction(*list(map(_operator.invert, self)))
...
class Disjunction(Condition, tuple):
    ...
    def __invert__(self):
        return Conjunction(*list(map(_operator.invert, self)))
